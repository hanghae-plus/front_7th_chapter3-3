...
import { useEditPost } from "../features/edit-post"
import { useDeletePost } from "../features/delete-post"
import { useAddComment } from "../features/add-comment"
import { useEditComment } from "../features/edit-comment"

// Entities
import { usePostStore, Post } from "../entities/post"
import { useCommentStore } from "../entities/comment"
import { useUserStore } from "../entities/user"

// Widgets
import { PostTable, Pagination, UserModal } from "../widgets/post-list"
...
const PostsManager = () => {
  ...
  // Comment Store
  const { commentsByPostId, fetchComments, likeComment, deleteComment } = useCommentStore()

  // User Store
  const { selectedUser, showUserModal, openUserModal, setShowUserModal } = useUserStore()

  // Features - Post
  const { showDialog: showAddDialog, newPost, setShowDialog: setShowAddDialog, setNewPost, addPost } = useAddPost()
  ...
  const {
    ...
    newComment,
    setShowDialog: setShowAddCommentDialog,
    setNewComment,
    openDialog: openAddCommentDialog,
    addComment,
  } = useAddComment()

  const {
    showDialog: showEditCommentDialog,
    selectedComment,
    setShowDialog: setShowEditCommentDialog,
    setSelectedComment,
  ...
  const fetchPosts = () => {
    ...
    let usersData: { id: number; username: string; image: string }[]
  ...
  // 작성자 클릭 핸들러
  const handleAuthorClick = (author: { id: number }) => {
    ...
    <Card className="w-full max-w-6xl mx-auto">
            ...
            <div className="flex-1">
              <div className="relative">
                ...
                <Input
                  ...
                  onKeyPress={(e: KeyboardEvent<HTMLInputElement>) => e.key === "Enter" && searchPosts()}
                />
              </div>
            </div>
            <Select
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <Textarea
              ...
              onChange={(e: ChangeEvent<HTMLTextAreaElement>) => setNewComment({ ...newComment, body: e.target.value })}
            />
      ...
      <Dialog open={showPostDetailDialog} onOpenChange={setShowPostDetailDialog}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            <DialogTitle>{highlightText(detailPost?.title || "", searchQuery)}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <p>{highlightText(detailPost?.body || "", searchQuery)}</p>
            {detailPost && renderComments(detailPost.id)}
          </div>
        </DialogContent>
      </Dialog>
